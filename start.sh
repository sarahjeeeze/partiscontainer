...
python /partis/additionalScripts/germlineToPartis.py $GERMLINE_BUILD_DIR/germlines/human $SPECIES
...
for filename in "$INPUT_DIR"/*; do
	...
	echo ""
		echo ""
		COMMAND="python ../partis/bin/partis annotate --infname /partis/sampledata/input/sample.fasta --initial-germline-dir $GERMLINE_BUILD_DIR/germlines/partis --locus $RECEPTOR --outfname $OUTPUT_DIR/partis.csv"
		echo ""
		eval $COMMAND
	done
